...
const BridgePage = () => {
  ...
  useEffect(() => {
    const token = searchParams.get('central_token') || searchParams.get('token');
    ...
    const performLogin = async () => {
      try {
        ...
        localStorage.setItem('mprar_central_token', bridgeToken);

        await loginWithToken(bridgeToken);
        navigate('/', { replace: true });
      } catch (err) {
        console.error('Bridge login failed:', err);
        const detail = err?.message || 'Unknown authentication error';
        setError(`Authentication failed: ${detail}. Please try launching again from the Central Platform.`);
      }
    };
